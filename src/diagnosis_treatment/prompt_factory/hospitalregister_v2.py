...
@register_prompt
class PromptHospitalRegister_v2(PromptTemplate):
...
作为<Role>，任务为<Profile>，按<Workflow>的顺序和患者对话。"""
    ...
    def __set_recognize_intent_by_intention_model(self):
        ...
        system_str=f"""#你是一名医院挂号员，你的角色是对患者的话语进行意图分析，并生成对应的意图结果，意图结果需严格匹配预设的意图表。
#基础信息
-所有科室：
{self.current_department}。
-所有医生：
...
## Input
-当前输入的患者话语为：“{self.patient_intent}”
-历史意图为：“{self.last_system_intent}”
##### 配置意图表
- **科室**：某科（仅当用户明确要换科室时才更改，需在后处理中进行模糊匹配）
- **医生职称**：主任，专家
- **医生姓名**：某医生
- **挂号日期**：最近，某天，某天后，某天到某天，某天前
    -标准化输出格式：
        - 最近：直接输出 \"最近\"
        - 某天：输出具体的日期，格式为 YYYY.MM.DD
        - 某天后：计算并输出具体日期，格式为 [YYYY.MM.DD - )
...
- **号源**：最多，最少
- **意图类型**：更改挂号，取消挂号，确认挂号，查询
- **查询主体**：医生姓名，号源，挂号日期，医生特长

## 推理与处理步骤
1. 梳理用户历史对话：对患者此前的每一次发言进行深度挖掘，详细提取并妥善记录科室、医生姓名、医生职称、挂号日期、挂号时间等关键信息。
2. 分析当前发言，明确其核心意图。
    - 确认挂号意图：一旦患者话语中出现 “可以、好的、就这个号” 等确认性表述，立即将意图类型判定为确认挂号。
    - 时间冲突处理：当患者无法按意图中的挂号日期和时间就诊，例如说出 “我下午没空” 或者 “我上午有事” 这类表述时，需要将意图结果中的挂号日期往后顺延一天，挂号时间调整为当天对应的相反时段（若原是上午则调整为下午，原是下午则调整为上午）。举例来说，如果原本挂号日期为 2025.04.10 且挂号时间为下午，当患者表示 “我下午没空” 时，调整后的挂号日期应为 2025.04.11，挂号时间为上午。
    - 医生姓名、挂号日期、挂号时间字段缺失处理：以历史信息和已有挂号信息为依据，生成具体的医生、日期、时间信息。
...
3. 生成意图结果：始终按照限定条件从多到少的顺序，生成3个意图结果：
    -1.第一个意图结果必须同时包含医生姓名、挂号日期、挂号时间三个字段
    -2.第二个意图结果必须包含医生姓名，且涵盖尽可能多其他关键信息（如挂号日期、挂号时间）的意图结果。
    -3.第三个意图结果生成限定条件较少、更宽泛的意图结果，严格检查生成的意图结果，去除重复或无实质差异的结果。
4. 输出格式：输出格式为JSON数组，所有字段示例如下：\"{{\"意图类型\": \"\",\"科室名称\": \"\",\"医生姓名\": \"\",\"医生职称\": \"\",\"挂号日期\": \"\",\"挂号时间\": \"\",\"号源\": \"\",\"查询主体\": \"}}\"的格式返回;
返回示例： \"{{\"意图类型\": \"更改挂号\",\"医生姓名\": \"张三\",\"挂号日期\": \"2025.03.10\",\"挂号日期\": \"最近\"}}\"。
